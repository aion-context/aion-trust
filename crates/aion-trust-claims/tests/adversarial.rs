...
fn setup() -> Setup {
    ...
    let validity = Validity { from: Timestamp(1_600_000_000), until: None };
    ...
    Setup { subject, verifier, dir, claim, now }
}

fn present(s: &Setup, signer: &Identity, claims: Vec<Claim>) -> Presentation {
    build_presentation(
        signer, &s.verifier, "application:senior-engineer",
        b"nonce-abcdef012345", s.now, s.now.plus_seconds(3600), claims,
    )
}
...
fn forged_claim_signature_is_rejected() {
    let s = setup();
    let mut v = serde_json::to_value(&s.claim).unwrap();
    let bad: String = v["issuer_signature"].as_str().unwrap().chars().rev().collect();
...
fn expired_presentation_is_rejected() {
    ...
    let p = build_presentation(
        &s.subject, &s.verifier, "application", b"nonce-1",
        s.now, s.now.plus_seconds(10), vec![s.claim.clone()],
    );
...
fn claim_outside_its_validity_window_is_rejected() {
    ...
    let subject = Identity::generate();
    let verifier = Identity::generate().did();
    let validity = Validity { from: Timestamp(1_000), until: Some(Timestamp(2_000)) };
    let claim = Claim::issue(&issuer, &subject.did(), validity, emp_body()).unwrap();
    let mut dir = IssuerDirectory::new();
    dir.register(issuer.verifying_key());
    let now = Timestamp(5_000);
    let p = build_presentation(
        &subject, &verifier, "application", b"nonce-2", now, now.plus_seconds(60), vec![claim],
    );
